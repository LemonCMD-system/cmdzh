...
def run_cmd(command: str, wait: bool = True):
    try:
        ...
        return -1
def main():
    print("中文版命令行开源版 [版本3.6.26213.1632]")
    ...
    while True:
        ...
        if user_input_trim.startswith("删除目录 "):
            ...
            run_cmd(f"rd {param}")
            continue
        if user_input_trim == "目录树显示":
            run_cmd("tree")
            continue
        if user_input_trim.startswith("目录树显示 "):
            param = trim_string(user_input_trim[5:])
            run_cmd(f"tree {param}")
            continue
        # 文件操作类
        ...
        if user_input_trim.startswith("重命名文件 "):
            ...
            run_cmd(f"ren {param}")
            continue
        if user_input_trim == "复制目录树":
            print("[提示] 复制目录树用法：复制目录树 源路径 目标路径（如：复制目录树 D:\文档 E:\备份）")
            print("输入【帮助 复制目录树】查看详细用法")
            continue
        if user_input_trim.startswith("复制目录树 "):
            param = trim_string(user_input_trim[5:])
            run_cmd(f"xcopy {param}")
            continue
        if user_input_trim == "高级复制":
            print("[提示] 高级复制用法：高级复制 源路径 目标路径（如：高级复制 D:\文档 E:\备份）")
            print("输入【帮助 高级复制】查看详细用法")
            continue
        if user_input_trim.startswith("高级复制 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"robocopy {param}")
            continue
        if user_input_trim == "替换文件":
            print("[提示] 替换文件用法：替换文件 源文件 目标路径（如：替换文件 1.txt D:\数据）")
            print("输入【帮助 替换文件】查看详细用法")
            continue
        if user_input_trim.startswith("替换文件 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"replace {param}")
            continue
        if user_input_trim == "查看文件内容":
            print("[提示] 查看文件内容用法：查看文件内容 文件名（如：查看文件内容 文档.txt）")
            print("输入【帮助 查看文件内容】查看详细用法")
            continue
        if user_input_trim.startswith("查看文件内容 "):
            param = trim_string(user_input_trim[6:])
            run_cmd(f"type {param}")
            continue
        if user_input_trim == "恢复文件":
            print("[提示] 恢复文件用法：恢复文件 文件名（如：恢复文件 损坏.txt）")
            print("输入【帮助 恢复文件】查看详细用法")
            continue
        if user_input_trim.startswith("恢复文件 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"recover {param}")
            continue
        # 文件管理类
        if user_input_trim == "文件关联":
            run_cmd("assoc")
            continue
        if user_input_trim.startswith("文件关联 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"assoc {param}")
            continue
        if user_input_trim == "文件属性":
            run_cmd("attrib")
            continue
        if user_input_trim.startswith("文件属性 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"attrib {param}")
            continue
        if user_input_trim == "比较文件":
            print("[提示] 比较文件用法：比较文件 文件1 文件2（如：比较文件 1.txt 2.txt）")
            print("输入【帮助 比较文件】查看详细用法")
            continue
        if user_input_trim.startswith("比较文件 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"comp {param}")
            continue
        if user_input_trim == "文件比较":
            print("[提示] 文件比较用法：文件比较 文件1 文件2（如：文件比较 1.txt 2.txt）")
            print("输入【帮助 文件比较】查看详细用法")
            continue
        if user_input_trim.startswith("文件比较 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"fc {param}")
            continue
        if user_input_trim == "NTFS压缩":
            run_cmd("compact")
            continue
        if user_input_trim.startswith("NTFS压缩 "):
            param = trim_string(user_input_trim[5:])
            run_cmd(f"compact {param}")
            continue
        if user_input_trim == "转换分区格式":
            print("[提示] 转换分区格式用法：转换分区格式 盘符: /fs:ntfs（如：转换分区格式 D: /fs:ntfs）")
            print("输入【帮助 转换分区格式】查看详细用法")
            continue
        if user_input_trim.startswith("转换分区格式 "):
            param = trim_string(user_input_trim[6:])
            run_cmd(f"convert {param}")
            continue
        if user_input_trim == "查找文本":
            print("[提示] 查找文本用法：查找文本 \"字符串\" 文件名（如：查找文本 \"测试\" 1.txt）")
            print("输入【帮助 查找文本】查看详细用法")
            continue
        if user_input_trim.startswith("查找文本 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"find {param}")
            continue
        if user_input_trim == "查找字符串":
            print("[提示] 查找字符串用法：查找字符串 \"字符串\" 文件名（如：查找字符串 \"测试\" 1.txt）")
            print("输入【帮助 查找字符串】查看详细用法")
            continue
        if user_input_trim.startswith("查找字符串 "):
            param = trim_string(user_input_trim[5:])
            run_cmd(f"findstr {param}")
            continue
        if user_input_trim == "格式化磁盘":
            print("[提示] 格式化磁盘用法：格式化磁盘 盘符:（如：格式化磁盘 D:），谨慎使用！")
            print("输入【帮助 格式化磁盘】查看详细用法")
            continue
        if user_input_trim.startswith("格式化磁盘 "):
            param = trim_string(user_input_trim[5:])
            run_cmd(f"format {param}")
            continue
        if user_input_trim == "文件系统配置":
            print("[提示] 文件系统配置用法：文件系统配置 fsinfo drives（显示所有驱动器）")
            print("输入【帮助 文件系统配置】查看详细用法")
            continue
        if user_input_trim.startswith("文件系统配置 "):
            param = trim_string(user_input_trim[6:])
            run_cmd(f"fsutil {param}")
            continue
        if user_input_trim == "文件类型关联":
            run_cmd("ftype")
            continue
        if user_input_trim.startswith("文件类型关联 "):
            param = trim_string(user_input_trim[6:])
            run_cmd(f"ftype {param}")
            continue
        if user_input_trim == "磁盘卷标":
            print("[提示] 磁盘卷标用法：磁盘卷标 盘符: 卷标名（如：磁盘卷标 D: 数据盘）")
            print("输入【帮助 磁盘卷标】查看详细用法")
            continue
        if user_input_trim.startswith("磁盘卷标 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"label {param}")
            continue
        if user_input_trim == "创建链接":
            print("[提示] 创建链接用法：创建链接 链接名 目标路径（如：创建链接 文档链接 D:\文档）")
            print("输入【帮助 创建链接】查看详细用法")
            continue
        if user_input_trim.startswith("创建链接 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"mklink {param}")
            continue
        if user_input_trim == "打开文件查询":
            run_cmd("openfiles /query")
            continue
        if user_input_trim.startswith("打开文件查询 "):
            param = trim_string(user_input_trim[5:])
            run_cmd(f"openfiles {param}")
            continue
        # 系统配置类
        if user_input_trim == "中断检查":
            run_cmd("break")
            continue
        if user_input_trim.startswith("中断检查 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"break {param}")
            continue
        if user_input_trim == "启动配置":
            print("[提示] 启动配置需管理员权限运行，输入【帮助 启动配置】查看详细用法")
            continue
        if user_input_trim.startswith("启动配置 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"bcdedit {param}")
            continue
        if user_input_trim == "访问控制列表":
            print("[提示] 访问控制列表用法：访问控制列表 文件名（如：访问控制列表 1.txt）")
            print("输入【帮助 访问控制列表】查看详细用法")
            continue
        if user_input_trim.startswith("访问控制列表 "):
            param = trim_string(user_input_trim[6:])
            run_cmd(f"icacls {param}")
            continue
        if user_input_trim == "代码页设置":
            run_cmd("chcp")
            continue
        if user_input_trim.startswith("代码页设置 "):
            param = trim_string(user_input_trim[5:])
            run_cmd(f"chcp {param}")
            continue
        if user_input_trim == "打开命令窗口":
            run_cmd("cmd", wait=False)
            continue
        if user_input_trim.startswith("打开命令窗口 "):
            param = trim_string(user_input_trim[5:])
            run_cmd(f"cmd {param}", wait=False)
            continue
        if user_input_trim == "驱动查询":
            run_cmd("driverquery")
            continue
        if user_input_trim.startswith("驱动查询 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"driverquery {param}")
            continue
        if user_input_trim == "设备配置":
            print("[提示] 设备配置用法：设备配置 con: cols=80 lines=25（设置控制台大小）")
            print("输入【帮助 设备配置】查看详细用法")
            continue
        if user_input_trim.startswith("设备配置 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"mode {param}")
            continue
        if user_input_trim == "命令提示设置":
            run_cmd("prompt")
            continue
        if user_input_trim.startswith("命令提示设置 "):
            param = trim_string(user_input_trim[5:])
            run_cmd(f"prompt {param}")
            continue
        if user_input_trim == "服务配置":
            print("[提示] 服务配置需管理员权限运行，输入【帮助 服务配置】查看详细用法")
            continue
        if user_input_trim.startswith("服务配置 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"sc {param}")
            continue
        if user_input_trim == "任务计划":
            run_cmd("schtasks /query")
            continue
        if user_input_trim.startswith("任务计划 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"schtasks {param}")
            continue
        if user_input_trim == "关闭计算机":
            print("[提示] 关闭计算机用法：关闭计算机 /s（关闭本地计算机）")
            print("输入【帮助 关闭计算机】查看详细用法")
            continue
        if user_input_trim.startswith("关闭计算机 "):
            param = trim_string(user_input_trim[5:])
            run_cmd(f"shutdown {param}")
            continue
        if user_input_trim == "排序输入":
            print("[提示] 排序输入用法：sort < 文件名（如：sort < 1.txt）")
            print("输入【帮助 排序输入】查看详细用法")
            continue
        if user_input_trim.startswith("排序输入 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"sort {param}")
            continue
        if user_input_trim == "启动程序":
            print("[提示] 启动程序用法：启动程序 程序名（如：启动程序 notepad）")
            print("输入【帮助 启动程序】查看详细用法")
            continue
        if user_input_trim.startswith("启动程序 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"start {param}", wait=False)
            continue
        if user_input_trim == "时间设置":
            run_cmd("time")
            continue
        if user_input_trim.startswith("时间设置 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"time {param}")
            continue
        if user_input_trim == "窗口标题设置":
            print("[提示] 窗口标题设置用法：窗口标题设置 标题名（如：窗口标题设置 我的终端）")
            print("输入【帮助 窗口标题设置】查看详细用法")
            continue
        if user_input_trim.startswith("窗口标题设置 "):
            param = trim_string(user_input_trim[6:])
            run_cmd(f"title {param}")
            continue
        if user_input_trim == "查看Windows版本":
            run_cmd("ver")
            continue
        if user_input_trim == "文件写入验证":
            run_cmd("verify")
            continue
        if user_input_trim.startswith("文件写入验证 "):
            param = trim_string(user_input_trim[6:])
            run_cmd(f"verify {param}")
            continue
        if user_input_trim == "磁盘卷标查看":
            run_cmd("vol")
            continue
        if user_input_trim.startswith("磁盘卷标查看 "):
            param = trim_string(user_input_trim[5:])
            run_cmd(f"vol {param}")
            continue
        if user_input_trim == "WMI信息查询":
            print("[提示] WMI信息查询用法：WMI信息查询 cpu get name（查看CPU名称）")
            print("输入【帮助 WMI信息查询】查看详细用法")
            continue
        if user_input_trim.startswith("WMI信息查询 "):
            param = trim_string(user_input_trim[5:])
            run_cmd(f"wmic {param}")
            continue
        # 系统高级类
        ...
        if user_input_trim.startswith("查看路径 "):
            ...
            continue
        if user_input_trim == "磁盘分区管理":
            print("[提示] 磁盘分区管理需管理员权限运行，输入【帮助 磁盘分区管理】查看详细用法")
            continue
        if user_input_trim.startswith("磁盘分区管理 "):
            param = trim_string(user_input_trim[5:])
            run_cmd(f"diskpart {param}")
            continue
        ...
        if user_input_trim == "打开任务管理器":
            ...
            continue
        if user_input_trim == "分屏显示":
            print("[提示] 分屏显示用法：more < 文件名（如：more < 1.txt）")
            print("输入【帮助 分屏显示】查看详细用法")
            continue
        if user_input_trim.startswith("分屏显示 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"more {param}")
            continue
        if user_input_trim == "暂停批处理":
            run_cmd("pause")
            continue
        # 批处理类
        if user_input_trim == "调用批处理":
            print("[提示] 调用批处理用法：调用批处理 批处理文件名（如：调用批处理 1.bat）")
            print("输入【帮助 调用批处理】查看详细用法")
            continue
        if user_input_trim.startswith("调用批处理 "):
            param = trim_string(user_input_trim[5:])
            run_cmd(f"call {param}")
            continue
        if user_input_trim == "回显":
            run_cmd("echo")
            continue
        if user_input_trim.startswith("回显 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"echo {param}")
            continue
        if user_input_trim == "结束本地环境":
            print("[提示] 结束本地环境用于批处理文件中，输入【帮助 结束本地环境】查看详细用法")
            continue
        if user_input_trim.startswith("结束本地环境 "):
            param = trim_string(user_input_trim[6:])
            run_cmd(f"endlocal {param}")
            continue
        if user_input_trim == "批处理循环":
            print("[提示] 批处理循环用法：for %i in (*.txt) do 命令 %i（如：for %i in (*.txt) do type %i）")
            print("输入【帮助 批处理循环】查看详细用法")
            continue
        if user_input_trim.startswith("批处理循环 "):
            param = trim_string(user_input_trim[5:])
            run_cmd(f"for {param}")
            continue
        if user_input_trim == "批处理跳转":
            print("[提示] 批处理跳转用于批处理文件中，输入【帮助 批处理跳转】查看详细用法")
            continue
        if user_input_trim.startswith("批处理跳转 "):
            param = trim_string(user_input_trim[5:])
            run_cmd(f"goto {param}")
            continue
        if user_input_trim == "组策略信息":
            run_cmd("gpresult /r")
            continue
        if user_input_trim.startswith("组策略信息 "):
            param = trim_string(user_input_trim[5:])
            run_cmd(f"gpresult {param}")
            continue
        if user_input_trim == "扩展字符集":
            print("[提示] 扩展字符集用法：扩展字符集 代码页号（如：扩展字符集 936）")
            print("输入【帮助 扩展字符集】查看详细用法")
            continue
        if user_input_trim.startswith("扩展字符集 "):
            param = trim_string(user_input_trim[5:])
            run_cmd(f"graftabl {param}")
            continue
        if user_input_trim == "批处理条件":
            print("[提示] 批处理条件用于批处理文件中，输入【帮助 批处理条件】查看详细用法")
            continue
        if user_input_trim.startswith("批处理条件 "):
            param = trim_string(user_input_trim[5:])
            run_cmd(f"if {param}")
            continue
        if user_input_trim == "恢复目录":
            print("[提示] 恢复目录用于批处理文件中，输入【帮助 恢复目录】查看详细用法")
            continue
        if user_input_trim.startswith("恢复目录 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"popd {param}")
            continue
        if user_input_trim == "保存目录":
            print("[提示] 保存目录用法：保存目录 目标路径（如：保存目录 D:\文档）")
            print("输入【帮助 保存目录】查看详细用法")
            continue
        if user_input_trim.startswith("保存目录 "):
            param = trim_string(user_input_trim[4:])
            run_cmd(f"pushd {param}")
            continue
        if user_input_trim == "开始本地环境":
            print("[提示] 开始本地环境用于批处理文件中，输入【帮助 开始本地环境】查看详细用法")
            continue
        if user_input_trim.startswith("开始本地环境 "):
            param = trim_string(user_input_trim[6:])
            run_cmd(f"setlocal {param}")
            continue
        if user_input_trim == "批处理参数调整":
            print("[提示] 批处理参数调整用于批处理文件中，输入【帮助 批处理参数调整】查看详细用法")
            continue
        if user_input_trim.startswith("批处理参数调整 "):
            param = trim_string(user_input_trim[6:])
            run_cmd(f"shift {param}")
            continue
