...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QTabWidget, QWidget, QVBoxLayout, QHBoxLayout,
                             ...
                             QDialog, QLabel, QSpinBox, QColorDialog, QFrame, QGridLayout)
from PyQt5.QtCore import Qt, QEvent
from PyQt5.QtCore import Qt, QUrl
from PyQt5.QtWidgets import QShortcut
from PyQt5.QtGui import QFont, QColor, QPalette
from PyQt5.QtWebEngineWidgets import QWebEngineView
CURRENT_VERSION = "V4.4.26314.0857"
GITHUB_RELEASE_API = "https://api.github.com/repos/LemonCMD-system/cmdzh/releases/latest"
...
def get_compatible_cmd(cmd):
    ...
    if system == "Linux":
        cmd_mapping = {
            ...
            "call": "source",
            "echo": "echo",
            "gpresult": "cat /etc/ssh/ssh_config",
            "graftabl": "locale charmap"
        }
        for win_cmd, linux_cmd in cmd_mapping.items():
            if cmd.startswith(win_cmd):
                cmd = cmd.replace(win_cmd, linux_cmd, 1)
                break
        if cmd.startswith("rmdir") and len(cmd) > 5:
            cmd = f"rm -r {cmd[5:]}"
    return cmd
class AppearanceSettingsDialog(QDialog):
    def __init__(self, parent=None, current_bg=DEFAULT_BG_COLOR, current_font=DEFAULT_FONT_COLOR, current_font_size=DEFAULT_FONT_SIZE):
        super().__init__(parent)
        self.setWindowTitle("外观设置")
        self.setModal(True)
        self.setMinimumSize(400, 300)
        self.original_bg = current_bg
        self.original_font = current_font
        self.original_font_size = current_font_size
    ...
    def init_ui(self):
        ...
        grid_layout.setContentsMargins(20, 20, 20, 10)
        main_layout = QVBoxLayout()
        grid_layout = QGridLayout()
        grid_layout.setSpacing(10)
        grid_layout.setContentsMargins(20, 20, 20, 10)
        self.bg_label = QLabel("背景色：")
        self.bg_display = QFrame()
        self.bg_display.setFixedSize(60, 25)
        self.bg_display.setStyleSheet(f"background-color: {self.current_bg}; border: 1px solid #cccccc;")
        self.bg_btn = QPushButton("选择颜色")
        self.bg_btn.clicked.connect(self.choose_bg_color)
        grid_layout.addWidget(self.bg_label, 0, 0)
        grid_layout.addWidget(self.bg_display, 0, 1)
        grid_layout.addWidget(self.bg_btn, 0, 2)
        self.font_color_label = QLabel("字体色：")
        self.font_color_display = QFrame()
        self.font_color_display.setFixedSize(60, 25)
        self.font_color_display.setStyleSheet(f"background-color: {self.current_font}; border: 1px solid #cccccc;")
        self.font_color_btn = QPushButton("选择颜色")
        self.font_color_btn.clicked.connect(self.choose_font_color)
        grid_layout.addWidget(self.font_color_label, 1, 0)
        grid_layout.addWidget(self.font_color_display, 1, 1)
        grid_layout.addWidget(self.font_color_btn, 1, 2)
        ...
        grid_layout.addWidget(self.font_size_label, 2, 0)
        grid_layout.addWidget(self.font_size_spin, 2, 1)
        preview_label = QLabel("效果预览：")
        self.preview_edit = QPlainTextEdit()
        self.preview_edit.setReadOnly(True)
        self.preview_edit.setFixedHeight(100)
        self.preview_edit.appendPlainText("中文版命令行 - 外观预览")
        self.preview_edit.appendPlainText("当前目录：/home/user > 查看目录")
        self.preview_edit.appendPlainText("π = 3.141592653589793")
        self.update_preview()
    ...
    def choose_bg_color(self):
        color = QColorDialog.getColor(QColor(self.current_bg), self, "选择背景色")
        if color.isValid():
            self.current_bg = color.name()
            self.bg_display.setStyleSheet(f"background-color: {self.current_bg}; border: 1px solid #cccccc;")
            self.update_preview()
    def choose_font_color(self):
        color = QColorDialog.getColor(QColor(self.current_font), self, "选择字体色")
        if color.isValid():
            self.current_font = color.name()
            self.font_color_display.setStyleSheet(f"background-color: {self.current_font}; border: 1px solid #cccccc;")
            self.update_preview()
    def update_preview(self):
        ...
        style = f"""
            QPlainTextEdit {{
                ...
                color: {self.current_font};
                font-size: {self.current_font_size}px;
                font-family: {"Consolas" if get_system() == "Windows" else "Monospace"};
                border: 1px solid #cccccc;
                padding: 5px;
            }}
        """
        self.preview_edit.setStyleSheet(style)
    def reset_to_default(self):
        self.current_bg = DEFAULT_BG_COLOR
        self.current_font = DEFAULT_FONT_COLOR
...
class TerminalTab(QWidget):
    def __init__(self, parent=None):
        ...
        self.output_edit.appendPlainText(f"中文版命令行开源版 [版本{CURRENT_VERSION}]")
    ...
    def update_style(self):
        ...
        """
        self.output_edit.setStyleSheet(style)
    def open_appearance_gui(self):
        dialog = AppearanceSettingsDialog(
            self,
            current_bg=self.bg_color,
            current_font=self.font_color,
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            self.update_style()
            self.show_prompt()
    def set_appearance(self, param):
        if not param:
            help_text = """
 外观设置（命令行）用法：
   1. 重置默认：外观设置 重置
   2. 设置背景色：外观设置 背景色 #xxxxxx（如：外观设置 背景色 #1e1e1e）
   3. 设置字体色：外观设置 字体色 #xxxxxx（如：外观设置 字体色 #00ff00）
   4. 设置字体大小：外观设置 字体大小 12（如：外观设置 字体大小 14，范围8-30）
...
"""
        ...
        if len(param_parts) < 1:
            self.output_edit.appendPlainText("外观设置参数错误！输入「外观设置」查看用法")
            return
        try:
            if param_parts[0] == "重置":
                self.bg_color = DEFAULT_BG_COLOR
                ...
                self.font_size = DEFAULT_FONT_SIZE
                self.update_style()
                self.output_edit.appendPlainText("外观已重置为默认设置：")
                self.output_edit.appendPlainText(f"背景色{self.bg_color}，字体色{self.font_color}，字体大小{self.font_size}号")
                return
            elif param_parts[0] == "背景色" and len(param_parts) >= 2:
                color = param_parts[1].strip()
                if re.match(r'^#[0-9a-fA-F]{6}$', color):
                    self.bg_color = color
                    self.update_style()
                    self.output_edit.appendPlainText(f"背景色已设置为{self.bg_color}")
                else:
                    self.output_edit.appendPlainText("背景色格式错误！需为#开头+6位十六进制（如#2d2d2d）")
            elif param_parts[0] == "字体色" and len(param_parts) >= 2:
                color = param_parts[1].strip()
                if re.match(r'^#[0-9a-fA-F]{6}$', color):
                    self.font_color = color
                    self.update_style()
                    self.output_edit.appendPlainText(f"字体色已设置为{self.font_color}")
                else:
                    self.output_edit.appendPlainText("字体色格式错误！需为#开头+6位十六进制（如#ffffff）")
            elif param_parts[0] == "字体大小" and len(param_parts) >= 2:
                size = param_parts[1].strip()
                if size.isdigit():
                    size_int = int(size)
                    if 8 <= size_int <= 30:
                        self.font_size = size_int
                        self.update_style()
                        self.output_edit.appendPlainText(f"字体大小已设置为{self.font_size}号")
                    else:
                        self.output_edit.appendPlainText("字体大小超出范围！需在8-30之间")
                else:
                    self.output_edit.appendPlainText("字体大小必须是数字（如10、12）")
            else:
                self.output_edit.appendPlainText("外观设置参数错误！输入「外观设置」查看用法")
        except Exception as e:
            self.output_edit.appendPlainText(f"外观设置失败：{str(e)}")
    ...
    def run_cmd(self, command: str, wait: bool = True):
        try:
            ...
            return -1
    def calculate_pi(self, precision=15):
        try:
            precision = int(precision)
            if precision < 1 or precision > 1000:
                self.output_edit.appendPlainText("[提示] π的精度范围支持1-1000位")
                return
            if precision > 15:
                import decimal
            ...
            self.output_edit.appendPlainText(f"π = {pi_str}")
        except ValueError:
            self.output_edit.appendPlainText("[提示] 精度必须是数字，如：计算π 50")
        except Exception as e:
            self.output_edit.appendPlainText(f"计算π失败：{str(e)}")
    def process_command(self):
        ...
        try:
            if command == "计算派":
                ...
                self.show_prompt()
                return
            if command.startswith("计算π "):
                param = trim_string(command[3:])
                self.calculate_pi(param)
                self.show_prompt()
                return
            if command == "外观设置GUI":
                self.open_appearance_gui()
                return
            if command == "外观设置":
                self.open_appearance_gui()
                return
            if command.startswith("外观设置 "):
                param = trim_string(command[4:])
                self.set_appearance(param)
                self.show_prompt()
                return
            if command == "检查更新":
                self.check_update()
            ...
            if command.startswith("设置背景颜色 "):
                ...
                color_input = trim_string(command[5:])
                if color_input in color_map:
                    self.bg_color = color_map[color_input]
                    self.update_style()
                    self.output_edit.appendPlainText(f"背景颜色已设置为：{color_input}（{self.bg_color}）")
                elif len(color_input) == 7 and color_input.startswith("#"):
                    try:
                        int(color_input[1:], 16)
                        self.bg_color = color_input
                        self.update_style()
                        self.output_edit.appendPlainText(f"背景颜色已设置为：{self.bg_color}")
                    except ValueError:
                        self.output_edit.appendPlainText("无效的颜色值，请输入有效的十六进制颜色（如#2d2d2d）或中文颜色名称（如深灰色）")
                else:
                    self.output_edit.appendPlainText("设置背景颜色用法：\n1. 设置背景颜色 中文颜色名称（如：设置背景颜色 深灰色）\n2. 设置背景颜色 #十六进制颜色值（如：设置背景颜色 #2d2d2d）\n支持的中文颜色名称：黑色、深灰色、浅灰色、白色、红色、绿色、蓝色、黄色、青色、紫色、橙色、粉色、深蓝色、浅绿色")
                self.show_prompt()
                return
            self.run_cmd(command)
            self.show_prompt()
        except Exception as e:
            self.output_edit.appendPlainText(f"执行命令失败：{str(e)}")
            self.output_edit.appendPlainText(traceback.format_exc())
            self.show_prompt()
    def check_update(self):
        self.output_edit.appendPlainText("正在检查更新...")
        try:
            headers = {
            ...
            latest_version = release_data.get("tag_name", "未知版本")
...
"""
                ...
                update_text += "当前已是最新版本！"
            self.output_edit.appendPlainText(update_text)
        except requests.exceptions.Timeout:
            self.output_edit.appendPlainText("检查更新超时,请检查您的网络环境")
        except requests.exceptions.SSLError:
            self.output_edit.appendPlainText("SSL证书验证失败,请验证网络环境")
        except Exception as e:
            self.output_edit.appendPlainText(f"检查更新失败：{str(e)}")
    def show_global_help(self):
        help_text = f"""
有关某个命令的详细信息，请键入 帮助 命令名
【目录操作类】
...
【自定义设置类】
  ...
  设置背景颜色    - 设置终端背景颜色，支持中文颜色名称和十六进制颜色值（如：设置背景颜色 深灰色 或 设置背景颜色 #2d2d2d）
【系统映像维护类】
...
class MainWindow(QMainWindow):
    ...
    def add_browser_tab(self):
        ...
        tab_index = self.tab_widget.addTab(browser_tab, "CMD-Zh内置浏览器")
        self.tab_widget.setCurrentIndex(tab_index)
    ...
    def create_menu(self):
        ...
        check_update_action.triggered.connect(self.check_update_gui)
        help_menu.addAction(check_update_action)
        appearance_menu = menubar.addMenu("外观")
        appearance_action = QAction("外观设置", self)
        appearance_action.triggered.connect(lambda: self.tab_widget.currentWidget().open_appearance_gui())
        appearance_menu.addAction(appearance_action)
    def check_update_gui(self):
        try:
            ...
            if reply == QMessageBox.Yes:
                if download_url and exe_filename:
                    try:
                        ...
                        import os
                        save_path = os.path.join(os.getcwd(), exe_filename)
                        with open(save_path, "wb") as f:
                            f.write(update_response.content)
                        QMessageBox.information(
                            self,
                            "下载成功",
                            f"已成功下载最新版本的exe文件：{exe_filename}\n请运行该文件完成更新。\n文件保存位置：{save_path}"
                        )
                    except Exception as e:
                        QMessageBox.critical(
                            self,
                            "下载失败",
                            f"自动下载失败：{str(e)}\n请手动前往下载：{download_url}"
                        )
                else:
                    QMessageBox.information(
                        self,
                        "无法自动下载",
                        f"发现新版本，但未找到可自动下载的exe文件，请手动前往下载：{download_url}"
                    )
            else:
                QMessageBox.information(
                    self,
            ...
            QMessageBox.warning(self, "检查更新失败", "检查更新超时，请检查网络连接")
        except Exception as e:
            QMessageBox.critical(self, "检查更新失败", f"检查更新出错：{str(e)}")
